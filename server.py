...
async def inter_treasury_transfer(
    request: Request,
    transfer: TreasuryTransferRequest,
    user: dict = Depends(require_permission(Modules.TREASURY, Actions.CREATE)),
):
...
async def get_vendors(
    ...
    for vendor in vendors:
        ...
        for tx in pending_txs:
            ...
            commission_base = (
                tx.get("vendor_commission_base_amount") or 0
            )  # FIX: None -> 0
        ...
        for ie in ie_by_vendor.get(vendor["vendor_id"], []):
            ...
            commission_base = (
                ie.get("vendor_commission_base_amount") or 0
            )  # FIX: None -> 0
...
async def get_all_conversations_admin(user: dict = Depends(get_current_user)):
    ...
    for conv in conversations:
        ...
        user1 = await db.users.find_one(
            {"user_id": user1_id}, {"_id": 0, "name": 1, "email": 1}
        )
        user2 = await db.users.find_one(
            {"user_id": user2_id}, {"_id": 0, "name": 1, "email": 1}
        )
    ...
    for msg in messages:
        ...
        if sender_id not in user_cache:
            sender = await db.users.find_one(
                {"user_id": sender_id}, {"_id": 0, "name": 1}
            )
